...
cell_types <- c("Adipocyte_cells", "Blood_immune_cells", "Endocrine_cells",
                "Endothelial_cells", "Germ_cells", "Glandular_cells", "Glial_cells",
                "Mesenchymal_cells", "Pigment_cells", "Specialized_epithelial_cells",
                "Trophoblast_cells")
...
values_in_germCells <- as_tibble(assay(scRNAseq_HPA), rownames = "ensembl_gene_id") %>%
  pivot_longer(names_to = "Cell_type", values_to = "counts", -ensembl_gene_id) %>%
  left_join(as_tibble(colData(scRNAseq_HPA))) %>%
...
rowdata <- as_tibble(rowData(scRNAseq_HPA), rownames = "ensembl_gene_id") %>%
  ...
  mutate(Higher_in_somatic_cell_type =
           case_when((max_TPM_in_a_somatic_cell_type > 0 | max_in_germcells_group > 0) &
                       max_TPM_in_a_somatic_cell_type > max_in_germcells_group ~ TRUE,
                     (max_TPM_in_a_somatic_cell_type > 0 | max_in_germcells_group > 0) &
                       max_TPM_in_a_somatic_cell_type <= max_in_germcells_group ~ FALSE,
                     (max_TPM_in_a_somatic_cell_type == 0 & max_in_germcells_group == 0) ~ NA)) %>%
  as.data.frame()
